...
def load_data():
    ...
    print(f"Найдены классы: {actual_classes}")
...
def create_model(num_classes):
    ...
    # AlexNet classifier состоит из 6 слоев, последний - Linear(4096, 1000)
    # Берем все слои кроме последнего
    new_classifier = net.classifier[:-1]

    # Добавляем новый последний слой с нужным количеством классов
...
def visualize_predictions(model, test_loader, class_names, device):
    ...
    # Просматриваем весь тестовый датасет
    model.eval()
    with torch.no_grad():
        for images, labels in test_loader:
            for i in range(len(labels)):
                class_idx = labels[i].item()
                # Берем по 3 изображения каждого класса
